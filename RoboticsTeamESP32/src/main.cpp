...
bool objectDebounce = false;

void ps3ReadPwmWrite(void * parameter); //task to read from ps3 controller and write to motor pwm
void continuosDetection(void * parameter); //task to read and pulse ultrasonic

unsigned long globalCurrentMillis; //current milliseconds variable for main program
unsigned long objectDetectPrevMillis; //previous milliseconds for object detect debounce timer
long objectDetectDebounceInterval = 1000; //millisecond interval for object detect debounce timer
...
void ps3ReadPwmWrite(void * parameter){//task to read from ps3 controller and write to motor pwm
  ...
  if(Ps3.event.analog_changed.stick.lx){
      ...
      if(Ps3.data.analog.stick.lx > ps3StickDeadzone){robotMove.rightShift(Ps3.data.analog.stick.lx);} //stick value greater than deadzon
...
void continuosDetection(void * parameter){//task to read and pulse ultrasonic
  for(;;){ //start infinite loop freeRTOS can break out of loop for other tasks
    ...
    if(sensor.objectDetected()){
      objectDebounce = true;
      objectDetectPrevMillis = millis();
    }
    else if (globalCurrentMillis - objectDetectDebounceInterval > objectDetectPrevMillis){objectDebounce = false;}
